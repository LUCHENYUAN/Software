...
class Comment(DBase):
    ...
    def insert_comment(self,post_id,content,ipaddr):
        ...
        comment=Comment(user_id=session.get('user_id'),user_name=session.get('user_name'),post_id=post_id,content=content,
    ...
    def get_comment_user_list(self,post_id,start,count):
        res=self.get_comment_with_user(post_id, start, count)
        comment_list=model_join_list(res)
